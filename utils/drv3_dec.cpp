...
BinaryData *srd_dec(BinaryData *data)
{
    ...
    while (true)
    {
        ...
        BinaryData *chunk = new BinaryData(data->get(chunk_cmp_size - 0x10));
    ...
    if (dec_size != result->Bytes.size())
    {
        // Size mismatch, something probably went wrong
        //throw ("Size mismatch: size was " + result->Bytes.size() + " but should be " + dec_size);
        throw "Size mismatch.";
    }
...
BinaryData *srd_dec_chunk(BinaryData *chunk, QString mode)
{
    BinaryData *result = new BinaryData();
    int data_size = chunk->Bytes.size();
    int flag = 1;
    int shift;
    ...
    while (chunk->Position < data_size)
    {
        ...
        if (b & 1)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                int reverse_index = result->Bytes.size() - offset;
                result->Bytes.append(result->Bytes[reverse_index]);
            }
        }
        else
